	...
	public void user_612_click_the_cardmenu_under_omni_corp_portal( ) {
		...
		try {
			us.clickOnElement(driver, 
					DataReader.locatorsMap.get("OmniScreen_Login").get("Cards_OmniHomeScreen"),false);
...
public void user_612_click_the_primaryaccount_dropdown_button() {
	for (int i = 0; i <100;i++) {
		try {
			us.clickOnElement(driver, DataReader.locatorsMap.get("Cards").get("DropdownArrow"), false);
			break;
		} catch (Exception e) {
			if (i==100) {
				Assert.fail(e.getMessage());
...
public void user_612_click_the_next_button_under_debit_card_request( ) {
	for (int i = 0; i < 100; i++) {
				try {
		us.clickOnElement(driver, DataReader.locatorsMap.get("Cards").get("Debit_Card_Request_next"), false);
	break;
} catch (Exception e) {
	if (i==100) {
		Assert.fail(e.getMessage());
...
public void user_612_click_the_maps_location( ) throws Throwable {
	...
	for (int i = 0; i < 200; i++) {
		try {
			us.clickOnElement(driver, DataReader.locatorsMap.get("Cards").get("DCR_MapsLoaction"), false);
			break;
...
public void user_612_select_the_account_under_the_primary_account_in_retail() throws Throwable {
	...
	//for (int i = 0; i <100; i++) {
				try {
		us.clickOnElement(driver, DataReader.locatorsMap.get("Cards").get("DCR_primaryAcct_Retail"),false);
	//	break;
	} catch (Exception e) {
//		if (i==100) {
//			   Assert.fail(e.getMessage());
